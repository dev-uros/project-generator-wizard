...
export default defineEventHandler(async (event) => {
  ...
  const originalFolderPath = join(process.cwd(), 'private-storage', 'nuxt-and-daisyui-website-template'); // Path to your private folder
  ...
  await populateFrontendWebsiteIndexName(tempFolderPath, projectNameNonFormatted)
...
const populateFrontendWebsiteIndexName = async (tempFolderPath: string, projectName: string) => {
  const baseHeaderFilePath = join(tempFolderPath, 'app', 'components', 'BaseHeader.vue');
  ...
  if (await fs.pathExists(baseHeaderFilePath)) {
    ...
    // Replace the placeholders with user inputs
    baseHeaderContent = baseHeaderContent
        .replace(/{{websiteName}}/g, projectName)
